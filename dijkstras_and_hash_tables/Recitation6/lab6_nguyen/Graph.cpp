...
void Graph::removeEdge(int key1, int key2)
{
  ...
  // TODO: Modify the adjacency lists (remember that the graph is undirected)
    ...
    // modifies the adjacency list for the key1 vertex
  ...
  // modifies the adjacency list for the key2 vertex
  for (int j = 0; j < v2->adj.size(); j++) {
        if(v2->adj[j].v->key == v1->key) {
...
bool Graph::isBridge(int key1, int key2)
{
  ...
  // counts the connected verticies
  for(int j = 0; j < vertices.size(); j++) {
      // if the vertex has been visied, increment the component counter by 1
  ...
  // counts the new number of connected components
  for(int j = 0; j < vertices.size(); j++) {
      // if the vertex has been visited, increment the componenets after removal counter by 1
      if(vertices[j]->visited) {
  ...
  // if the number of components has changed from removing the edge, return true

...
void timer(string filename) {
    ...
    ofstream out_file("insert_search_performance_hash_chained_" + filename);
    ...
    HashChain chainedHash;
    srand(time(NULL)); // seed for random number generator
    for (int i = 0; i < 100; i++) {
        int start = elemCount; // sets where to begin reading data from testData array
        chainedHash.resetCollisions(); // resets the number of collision after each iteration
        ...
        for(int j = start; j< start + 100; j++) {
            if(j < 10000) {
                ...
                chainedHash.insertItem(testData[j]); // insert test data to hash table
        ...
        for(int k = 0; k < 100; k++) {
            ...
            chainedHash.searchItem(testData[random(elemCount - 1)]);
        ...
        float avg_collisions = chainedHash.getNumOfCollision() / (100 * 1.0); // average number of collisions per 100 inserts/searches
